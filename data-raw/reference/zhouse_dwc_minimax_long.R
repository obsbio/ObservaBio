...
output_dir <- "outputs"
output_path <- file.path(output_dir, "dwc_zhouse_minimax_long.xlsx")
audit_output_path <- file.path(output_dir, "dwc_zhouse_auditoria_minimax_long.xlsx")
...
expand_project_operation <- function(project_raw, operation_raw) {
  projects <- split_semicolon(project_raw)
  operations <- split_semicolon(operation_raw)

  if (length(projects) == 0) {
    projects <- NA_character_
  }
  if (length(operations) == 0) {
    operations <- NA_character_
  }

  np <- length(projects)
  no <- length(operations)

  if (np == no) {
    return(tibble(projeto_item = projects, operacao_item = operations))
  }
  if (np == 1) {
    return(tibble(projeto_item = rep(projects, no), operacao_item = operations))
  }
  if (no == 1) {
    return(tibble(projeto_item = projects, operacao_item = rep(operations, np)))
  }

  # Avoid dropping information when both fields have multiple, unequal entries.
  tidyr::crossing(projeto_item = projects, operacao_item = operations)
}
...
dwc <- base_final %>%
  ...
  mutate(
    context_rows = map2(projeto_raw, operacao_raw, expand_project_operation)
  ) %>%
  unnest(context_rows) %>%
  mutate(
    datasetName = map_chr(projeto_item, project_piece_dataset),
    informationWithheld = map_chr(projeto_item, project_piece_withheld),
    locality = if_else(
      is_non_empty(operacao_item),
      str_squish(as.character(operacao_item)),
      NA_character_
    ),
    stateProvince = map_chr(locality, function(op) {
      if (!is_non_empty(op)) {
        return(NA_character_)
      }
      state <- unname(op_to_state[normalize_operation_key(op)])
  ...
  transmute(
    ...
    vernacularName = as.character(`Nome popular`),
...
if (!"statusIUCN" %in% template_cols) {
  insert_at <- match("statusSource", template_cols)
  if (is.na(insert_at)) {
    template_cols <- c(template_cols, "statusIUCN")
  } else {
    template_cols <- append(template_cols, "statusIUCN", after = insert_at)
  }
}
